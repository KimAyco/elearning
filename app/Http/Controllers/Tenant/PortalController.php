...
use App\Models\ClassDayProfile;
    ...
    use App\Models\User;
    use App\Models\LmsCourseCardCustomization;
    use App\Models\Discount;
    use App\Models\Scholarship;
    use App\Models\StudentWallet;
    use App\Models\LmsLesson;
    use App\Models\LmsClassroomMeeting;
    use App\Models\ProgramProspectus;
    use App\Models\ProgramProspectusItem;
    ...
    class PortalController extends Controller
    {
        ...
        public function lmsPage(Request $request): View
        {
            ...
            $courseCardCustomizations = [];
            if (! $weeklySessions->isEmpty()) {
                $classGroupIds = $weeklySessions->pluck('classGroup.id')->filter()->unique()->values()->all();
                $subjectIds = $weeklySessions->pluck('subject.id')->filter()->unique()->values()->all();

                $customizations = LmsCourseCardCustomization::query()
                    ->where('school_id', $schoolId)
                ...
                foreach ($customizations as $c) {
                    $key = 'cg' . (int) $c->class_group_id . '-sj' . (int) $c->subject_id;
                    $courseCardCustomizations[$key] = [
                        'color' => $c->accent_color,
                        'image' => $c->image_data_url,
                    ];
                }
            }

            return view('tenant.lms', compact('teachableSubjects', 'weeklySessions', 'courseCardCustomizations'));
        }

        public function saveLmsCourseCardAppearance(
            Request $request,
            ClassGroup $classGroup,
            Subject $subject
        ): JsonResponse {
            $roleCodes = (array) $request->attributes->get('role_codes', []);
            $permissionCodes = (array) $request->attributes->get('permission_codes', []);

            $isTeacherRole = in_array('teacher', $roleCodes, true);
            $hasManagePermission = in_array('teacher.content.manage', $permissionCodes, true);

            if (! $isTeacherRole && ! $hasManagePermission) {
                abort(403);
            }
            ...
            $accentColorRaw = $request->input('accent_color');
            $imageDataUrlRaw = $request->input('image_data_url');

            $accentColorRaw = is_string($accentColorRaw) ? trim($accentColorRaw) : '';
            $imageDataUrlRaw = is_string($imageDataUrlRaw) ? trim($imageDataUrlRaw) : '';

            $accentColor = $accentColorRaw !== '' ? $accentColorRaw : null;
            $imageDataUrl = $imageDataUrlRaw !== '' ? $imageDataUrlRaw : null;

            $isValidHexOrRgb = static function (?string $v): bool {
                if (! $v) return true;
                $hex = preg_match('/^#([0-9a-f]{3}|[0-9a-f]{6})$/i', $v) === 1;
                $rgb = preg_match('/^rgb\(\s*(\d{1,3}\s*,){2}\s*\d{1,3}\s*\)$/i', $v) === 1;
                return $hex || $rgb;
            };

            if (! $isValidHexOrRgb($accentColor)) $accentColor = null;
            if ($imageDataUrl !== null && mb_strlen($imageDataUrl) > 2000000) $imageDataUrl = null;

            // If both fields are empty, delete the customization (if any).
            if ($accentColor === null && $imageDataUrl === null) {
                LmsCourseCardCustomization::query()
                    ->where('school_id', $schoolId)
            ...
            LmsCourseCardCustomization::query()->updateOrCreate(
                [
                    'school_id' => $schoolId,
                ...
                [
                    ...
                    'created_by_user_id' => $request->attributes->get('actor_user_id'),
                ],
            );
        ...
        public function studentLessonsPage(Request $request, ClassGroup $classGroup, Subject $subject): View
        {
            ...
            $classroomMeeting = null;
            if (Schema::hasTable('lms_classroom_meetings')) {
                $classroomMeeting = LmsClassroomMeeting::query()
                    ->where('school_id', $schoolId)
                    ->where('class_group_id', (int) $classGroup->id)
                    ->where('subject_id', (int) $subject->id)
                    ->latest('created_at')
            ...
            $courseTeachers = $this->teachersForClassSubject($schoolId, $classGroup, $subject);

            return view('tenant.student-lessons', compact('classGroup', 'subject', 'modules', 'lessons', 'ungroupedQuizzes', 'classroomMeeting', 'courseTeachers'));
        }

        public function studentJoinClassroomMeet(Request $request, ClassGroup $classGroup, Subject $subject): RedirectResponse
        {
            $schoolId = (int) $request->attributes->get('active_school_id');
            $userId = (int) $request->attributes->get('actor_user_id');
            ...
            $meeting = LmsClassroomMeeting::query()
                ->where('school_id', $schoolId)
                ->where('class_group_id', (int) $classGroup->id)
                ->where('subject_id', (int) $subject->id)
            ...
            if ($meeting->hasScheduledMeetEnded()) {
                $back = url("/tenant/classes/{$classGroup->id}/{$subject->id}");

                return redirect($back)->with('status', 'This meeting has ended. The join link is no longer available.');
            }

            if (! $meeting->isOpenForStudentJoin()) {
                $back = url("/tenant/classes/{$classGroup->id}/{$subject->id}");
                $msg = 'The Google Meet link is only available during the scheduled session.';
                if ($meeting->scheduled_start && now()->lt($meeting->scheduled_start)) {
                    $msg = 'This meeting has not started yet. You can join when the scheduled time begins.';
                } elseif ($meeting->restrictsLateEntry()) {
                    $deadline = $meeting->studentJoinDeadline();
                    if ($deadline && now()->gt($deadline)) {
                        $msg = 'Your teacher limited how long after the start time students may join. That window has closed.';
                    }
                } elseif ($meeting->scheduled_start === null || $meeting->scheduled_end === null) {
                    $msg = 'This meeting is not scheduled yet. Ask your teacher when you can join.';
                }

                return redirect($back)->with('status', $msg);
            }

            return redirect()->away((string) $meeting->meet_link);
        }
        ...
        public function lmsClassDashboard(Request $request, ClassGroup $classGroup, Subject $subject): View
        {
            ...
            $googleAccountEmail = $this->syncGoogleAccountEmailForUser(User::find($userId));
            ...
            return view('tenant.lms-class-dashboard', [
                ...
                'classroomMeeting' => $classroomMeeting,
        ...
        $content = $payload['content'] ?? null;
            if ($type === 'link') {
            $rawLink = (string) ($payload['link_url'] ?? $payload['content'] ?? '');
            $content = $this->normalizeExternalUrl($rawLink);
                if ($content === null) {
                    return back()->withErrors(['content' => 'Please enter a valid URL (e.g. https://example.com).']);
                }
        } elseif ($type === 'doc') {
            $content = (string) ($payload['doc_content'] ?? $payload['content'] ?? '');
            }
        ...
        public function lmsUpdateModule(Request $request, LmsModule $module): RedirectResponse
        {
            $this->assertCanManageLmsModule($request, $module);
            ...
            if ($module->type === 'link') {
                $rules['content'] = ['required', 'string', 'max:2083', 'url'];
            } elseif ($module->type === 'doc') {
            ...
            if (in_array($module->type, ['link', 'doc'], true)) {
                if ($module->type === 'link') {
                    $normalized = $this->normalizeExternalUrl((string) $payload['content']);
                    if ($normalized === null) {
                        return back()->withErrors(['content' => 'Please enter a valid URL (e.g. https://example.com).']);
                    }
            ...
            if ($module->type === 'file' && $request->hasFile('file')) {
                $file = $request->file('file');
                $schoolId = (int) $module->school_id;
                $dir = "lms/{$schoolId}/class-{$module->class_group_id}/subject-{$module->subject_id}";
                $storedPath = Storage::disk('public')->putFile($dir, $file);
                $oldPath = $module->file_path;
                if (is_string($oldPath) && $oldPath !== '' && Storage::disk('public')->exists($oldPath)) {
                    Storage::disk('public')->delete($oldPath);
                }
                $updates['file_path'] = $storedPath;
                $updates['original_name'] = method_exists($file, 'getClientOriginalName') ? $file->getClientOriginalName() : null;
                $updates['mime_type'] = method_exists($file, 'getClientMimeType') ? $file->getClientMimeType() : null;
                $updates['file_size'] = method_exists($file, 'getSize') ? (int) ($file->getSize() ?? 0) : 0;
            }

            $module->update($updates);

            return redirect("/tenant/lms/classes/{$module->class_group_id}/{$module->subject_id}")
                ->with('status', 'Resource updated.');
        }

        public function lmsDestroyModule(Request $request, LmsModule $module): RedirectResponse
        {
            $this->assertCanManageLmsModule($request, $module);
            ...
            $classGroupId = (int) $module->class_group_id;
            $subjectId = (int) $module->subject_id;
            $module->delete();

            return redirect("/tenant/lms/classes/{$classGroupId}/{$subjectId}")
                ->with('status', 'Resource removed.');
        }

        public function lmsStoreClassroomMeeting(Request $request, ClassGroup $classGroup, Subject $subject): JsonResponse
        {
            $schoolId = (int) $request->attributes->get('active_school_id');
            $userId = (int) $request->attributes->get('actor_user_id');
            $roleCodes = (array) ($request->attributes->get('role_codes') ?? []);
            ...
            $payload = $request->validate([
                'title' => ['required', 'string', 'max:140'],
                'description' => ['nullable', 'string', 'max:2000'],
                'meet_link' => ['required', 'string', 'max:2083', 'url'],
            ]);

            $meeting = LmsClassroomMeeting::query()->create([
                'school_id' => $schoolId,
                'class_group_id' => (int) $classGroup->id,
                'subject_id' => (int) $subject->id,
                'title' => (string) $payload['title'],
                'description' => $payload['description'] ?? null,
                'meet_link' => (string) $payload['meet_link'],
        ...
        public function lmsGoogleAuth(Request $request): RedirectResponse
        {
            $classGroupId = $request->query('class_group_id');
            $subjectId = $request->query('subject_id');

            if ($classGroupId && $subjectId) {
                session(['google_oauth_return' => "/tenant/lms/classes/{$classGroupId}/{$subjectId}"]);
            }

            return redirect($this->googleMeetService->getAuthUrl());
        }

        public function lmsGoogleCallback(Request $request): RedirectResponse
        {
            $userId = (int) $request->attributes->get('actor_user_id');
            $returnUrl = session('google_oauth_return', '/tenant/lms');
            session()->forget('google_oauth_return');

            if ($request->has('error')) {
                return redirect($returnUrl)->with('error', 'Google authentication was cancelled.');
            }

            $code = $request->query('code');
            if (!$code) {
                return redirect($returnUrl)->with('error', 'Invalid Google authorization response.');
            }

            try {
                $token = $this->googleMeetService->authenticate($code);
                $user = User::findOrFail($userId);
                $this->googleMeetService->setAccessToken($token);
                $googleEmail = $this->googleMeetService->fetchGoogleAccountEmail();
                $finalToken = $this->googleMeetService->getClientAccessTokenArray() ?? $token;
                $user->update([
                    'google_token' => json_encode($finalToken),
                    'google_account_email' => $googleEmail,
                ]);

                return redirect($returnUrl)->with('status', 'Google Calendar connected successfully!');
            } catch (\Exception $e) {
                return redirect($returnUrl)->with('error', 'Failed to connect Google Calendar: ' . $e->getMessage());
            }
        }

        public function lmsCreateClassroomMeetingWithGoogle(Request $request, ClassGroup $classGroup, Subject $subject): JsonResponse
        {
            $schoolId = (int) $request->attributes->get('active_school_id');
            $userId = (int) $request->attributes->get('actor_user_id');
            $roleCodes = (array) ($request->attributes->get('role_codes') ?? []);
            ...
            $payload = $request->validate([
                ...
                'late_entry_minutes' => ['nullable', 'integer', 'min:1', 'max:300'],
            ]);

            $user = User::findOrFail($userId);

            if (!$this->googleMeetService->isTokenValid($user)) {
                return response()->json([
                    'ok' => false,
                    'needs_auth' => true,
                    'auth_url' => url("/tenant/lms/google/auth?class_group_id={$classGroup->id}&subject_id={$subject->id}"),
                    'message' => 'Please connect your Google Calendar first.',
                ], 401);
            }

            try {
                $result = $this->googleMeetService->createMeetingWithLink(
                    $user,
                    $payload['title'],
                    $payload['description'] ?? '',
                    $payload['start_time'],
                ...
                $meeting = LmsClassroomMeeting::query()->create([
                    ...
                    'late_entry_minutes' => $payload['late_entry_minutes'] ?? null,
        ...
        public function lmsCheckGoogleConnection(Request $request): JsonResponse
        {
            $userId = (int) $request->attributes->get('actor_user_id');
            $user = User::find($userId);

            $connected = $user && $this->googleMeetService->isTokenValid($user);
            $googleAccountEmail = $connected ? $this->syncGoogleAccountEmailForUser($user) : null;
        ...
        public function lmsUpdateLesson(Request $request, LmsLesson $lesson): RedirectResponse
        {
            $this->assertCanManageLmsLesson($request, $lesson);

            $payload = $request->validate([
                'title' => ['required', 'string', 'max:140'],
            ]);

            $lesson->update(['title' => $payload['title']]);

            return redirect("/tenant/lms/classes/{$lesson->class_group_id}/{$lesson->subject_id}")
                ->with('status', 'Section updated.');
        }

        public function lmsReorderLessons(Request $request, ClassGroup $classGroup, Subject $subject): RedirectResponse
        {
            $schoolId = (int) $request->attributes->get('active_school_id');
            $userId = (int) $request->attributes->get('actor_user_id');
            $roleCodes = (array) ($request->attributes->get('role_codes') ?? []);
            ...
            // Keep ordering, but ensure uniqueness.
            $lessonIds = array_values(array_unique(array_map(static fn ($v): int => (int) $v, $payload['lesson_ids'])));

            $lessonCount = LmsLesson::query()
                ->where('school_id', $schoolId)
                ->where('class_group_id', (int) $classGroup->id)
                ->where('subject_id', (int) $subject->id)
            ...
            if ($lessonCount !== count($lessonIds)) {
                return redirect("/tenant/lms/classes/{$classGroup->id}/{$subject->id}")
                    ->withErrors(['lessons' => 'Invalid section list for reorder.']);
            }

            // Re-position in the order received (1..N).
        ...
        public function lmsDestroyLesson(Request $request, LmsLesson $lesson): RedirectResponse
        {
            $this->assertCanManageLmsLesson($request, $lesson);

            $schoolId = (int) $request->attributes->get('active_school_id');
            $classGroupId = (int) $lesson->class_group_id;
            ...
            LmsQuiz::query()
                ...
                ->update([
                    'lesson_id' => null,
                    // If a section is removed, hide its quizzes from students
                    // until the teacher explicitly republishes them.
                    'is_published' => false,
                ]);
        ...
        private function assertCanManageLmsLesson(Request $request, LmsLesson $lesson): void
        {
            $schoolId = (int) $request->attributes->get('active_school_id');
            $userId = (int) $request->attributes->get('actor_user_id');
            $roleCodes = (array) ($request->attributes->get('role_codes') ?? []);
            ...
            if ((int) $lesson->school_id !== $schoolId) {
                abort(404);
            }
            ...
            $hasTeachingAssignment = ClassSession::query()
                ->where('school_id', $schoolId)
                ->where('teacher_user_id', $userId)
                ->where('class_group_id', (int) $lesson->class_group_id)
                ->where('subject_id', (int) $lesson->subject_id)
                ->exists();
        ...
        private function assertCanManageLmsModule(Request $request, LmsModule $module): void
        {
            $schoolId = (int) $request->attributes->get('active_school_id');
            $userId = (int) $request->attributes->get('actor_user_id');
            $roleCodes = (array) ($request->attributes->get('role_codes') ?? []);
            ...
            $hasTeachingAssignment = ClassSession::query()
                ->where('school_id', $schoolId)
                ->where('teacher_user_id', $userId)
                ->where('class_group_id', (int) $module->class_group_id)
                ->where('subject_id', (int) $module->subject_id)
                ->exists();
        ...
        public function lmsViewModule(Request $request, LmsModule $module)
        {
            $schoolId = (int) $request->attributes->get('active_school_id');
            if ((int) $module->school_id !== $schoolId) {
                abort(404);
            ...
            $mime = (string) ($module->mime_type ?: mime_content_type($absolutePath) ?: 'application/octet-stream');
            $inlineName = $module->original_name ?: basename($path);
        ...
        /**
         * Calendar events are created on this Google account; teachers must open Meet signed in as this user to be host.
         */
        private function syncGoogleAccountEmailForUser(?User $user): ?string
        {
            if (! $user || empty($user->google_token)) {
                return $user?->google_account_email;
            }
            if (filled($user->google_account_email)) {
                return $user->google_account_email;
            }
            if (! $this->googleMeetService->isTokenValid($user)) {
                return null;
            ...
            try {
                $tokenArray = is_string($user->google_token) ? json_decode($user->google_token, true) : $user->google_token;
                if (! is_array($tokenArray)) {
                    return null;
                }
                $before = $tokenArray;
                $this->googleMeetService->setAccessToken($tokenArray);
                $email = $this->googleMeetService->fetchGoogleAccountEmail();
                $this->googleMeetService->persistRefreshedTokenIfChanged($user, $before);
                if ($email) {
                    $user->forceFill(['google_account_email' => $email])->saveQuietly();
                }

                return $email ?? $user->fresh()->google_account_email;
            } catch (\Throwable $e) {
                return $user->google_account_email;
            }
        }
        ...
        /**
         * Teachers linked to this class + subject via scheduled class sessions.
         *
         * @return \Illuminate\Support\Collection<int, User>
         */
        private function teachersForClassSubject(int $schoolId, ClassGroup $classGroup, Subject $subject)
        {
            return ClassSession::query()
                ...
                ->with('teacher')
                ->get()
                ->map(fn (ClassSession $session): ?User => $session->teacher)
                ->filter()
                ->unique('id')
                ->values();
        }
        ...
        public function classPage(Request $request): View
        {
            ...
            $courseCardCustomizations = [];
            if ($classSubjectRows->isNotEmpty()) {
                $classGroupIds = $classSubjectRows->pluck('class_group_id')->filter()->unique()->values()->all();
                $subjectIds = $classSubjectRows->pluck('subject_id')->filter()->unique()->values()->all();
            ...
            return view('tenant.class', compact('classSubjectRows', 'courseCardCustomizations'));
        }
        ...
        public function enrollmentsPage(Request $request): View
        {
            ...
            // Student weekly schedule table pagination (10 rows/page)
            $studentWeeklyScheduleTable = new LengthAwarePaginator(
                items: $studentWeeklyScheduleRows
                    ->forPage((int) $request->query('student_schedule_page', 1), 10)
                    ->values(),
                total: $studentWeeklyScheduleRows->count(),
                perPage: 10,
                currentPage: max(1, (int) $request->query('student_schedule_page', 1)),
                options: [
                    'path' => $request->url(),
                    'query' => $request->query(),
                    'pageName' => 'student_schedule_page',
                ]
            );
            ...
            $registrarVerifiedSearch = trim((string) $request->query('registrar_verified_q', ''));
            $registrarClearanceSearch = trim((string) $request->query('registrar_clearance_q', ''));
            $registrarClassGroupSearch = trim((string) $request->query('registrar_class_group_q', ''));
            $registrarInactiveSearch = trim((string) $request->query('registrar_inactive_q', ''));
            $registrarAllStudentsSearch = trim((string) $request->query('registrar_all_students_q', ''));
            $registrarStaffSearch = trim((string) $request->query('registrar_staff_q', ''));
            ...
            if ($canRegistrarViewAllRecords) {
                ...
                $pendingClearancesForRegistrarQuery = Billing::query()
                ...
                if ($registrarClearanceSearch !== '') {
                    $escaped = addcslashes($registrarClearanceSearch, '%_\\');
                    $like = '%' . $escaped . '%';
                    $pendingClearancesForRegistrarQuery->where(function ($query) use ($like, $registrarClearanceSearch): void {
                        $query->where('description', 'like', $like)
                            ->orWhereHas('student', function ($studentQuery) use ($like): void {
                                $studentQuery->where('full_name', 'like', $like);
                            });

                        if (is_numeric($registrarClearanceSearch)) {
                            $id = (int) $registrarClearanceSearch;
                            $query->orWhere('id', $id)
                                ->orWhere('student_user_id', $id);
                        }
                    });
                }

                $pendingClearancesForRegistrar = $pendingClearancesForRegistrarQuery
                    ->paginate(10, ['*'], 'registrar_clearance_page')
                    ->appends($request->except('registrar_clearance_page'));

                $financeVerifiedForRegistrarQuery = Enrollment::query()
                ...
                if ($registrarVerifiedSearch !== '') {
                    $escaped = addcslashes($registrarVerifiedSearch, '%_\\');
                    $like = '%' . $escaped . '%';
                    $financeVerifiedForRegistrarQuery->where(function ($query) use ($like, $registrarVerifiedSearch): void {
                        $query->whereHas('student', function ($studentQuery) use ($like): void {
                            $studentQuery->where('full_name', 'like', $like);
                        });

                        if (is_numeric($registrarVerifiedSearch)) {
                            $id = (int) $registrarVerifiedSearch;
                            $query->orWhere('id', $id)
                                ->orWhere('student_user_id', $id);
                        }
                    });
                ...
                $financeVerifiedForRegistrar = $financeVerifiedForRegistrarQuery
                    ->paginate(10, ['*'], 'registrar_verified_page')
                    ->appends($request->except('registrar_verified_page'));

                $classGroupCapacityQuery = ClassGroup::query()
                    ->with(['program', 'semester'])
                    ->withCount(['studentAssignments as students_inside_count'])
                    ->where('school_id', $schoolId)
                    ->orderByDesc('id');

                if ($registrarClassGroupSearch !== '') {
                    $escaped = addcslashes($registrarClassGroupSearch, '%_\\');
                    $like = '%' . $escaped . '%';
                    $classGroupCapacityQuery->where(function ($query) use ($like): void {
                        $query->where('name', 'like', $like)
                            ->orWhereHas('program', function ($programQuery) use ($like): void {
                                $programQuery->where('code', 'like', $like)
                                    ->orWhere('name', 'like', $like);
                            });
                    });
                }

                $classGroupCapacityForRegistrar = $classGroupCapacityQuery
                    ->paginate(10, ['*'], 'registrar_class_group_page')
                    ->appends($request->except('registrar_class_group_page'));
                ...
                $inactiveStudentAccountsQuery = User::query()
                    ->whereIn('id', $studentUserIds->all())
                    ->whereIn('id', $verifiedBillingStudentIds->all())
                    ->where('status', '!=', 'active')
                    ->orderBy('full_name');

                if ($registrarInactiveSearch !== '') {
                    $escaped = addcslashes($registrarInactiveSearch, '%_\\');
                    $like = '%' . $escaped . '%';
                    $inactiveStudentAccountsQuery->where(function ($q) use ($like, $registrarInactiveSearch): void {
                        $q->where('full_name', 'like', $like);
                        if (is_numeric($registrarInactiveSearch)) {
                            $q->orWhere('id', (int) $registrarInactiveSearch);
                        }
                    });
                }

                $inactiveStudentAccountsForRegistrar = $inactiveStudentAccountsQuery
                    ->paginate(10, ['*'], 'registrar_inactive_page')
                    ->appends($request->except('registrar_inactive_page'));

                $allStudentsForRegistrarQuery = User::query()
                    ->whereIn('id', $studentUserIds->all())
                    ->orderBy('full_name');

                if ($registrarAllStudentsSearch !== '') {
                    $escaped = addcslashes($registrarAllStudentsSearch, '%_\\');
                    $like = '%' . $escaped . '%';
                    $allStudentsForRegistrarQuery->where(function ($q) use ($like, $registrarAllStudentsSearch): void {
                        $q->where('full_name', 'like', $like);
                        if (is_numeric($registrarAllStudentsSearch)) {
                            $q->orWhere('id', (int) $registrarAllStudentsSearch);
                        }
                    });
                }

                $allStudentsForRegistrar = $allStudentsForRegistrarQuery
                    ->paginate(10, ['*'], 'registrar_all_students_page')
                    ->appends($request->except('registrar_all_students_page'));

                $schoolStaffQuery = User::query()
                    ->whereIn('id', function ($sub) use ($schoolId): void {
                ...
                if ($registrarStaffSearch !== '') {
                    $escaped = addcslashes($registrarStaffSearch, '%_\\');
                    $like = '%' . $escaped . '%';
                    $schoolStaffQuery->where(function ($q) use ($like, $registrarStaffSearch): void {
                        $q->where('full_name', 'like', $like);
                        if (is_numeric($registrarStaffSearch)) {
                            $q->orWhere('id', (int) $registrarStaffSearch);
                        }
                    });
                }

                $schoolStaffPaginator = $schoolStaffQuery
                    ->paginate(10, ['*'], 'registrar_staff_page')
                    ->appends($request->except('registrar_staff_page'));

                $staffUserIds = $schoolStaffPaginator->getCollection()->pluck('id')->map(fn ($id): int => (int) $id)->all();
                $roleRowsByUserId = collect();
                if ($staffUserIds !== []) {
                    $roleRowsByUserId = UserRole::query()
                        ->join('roles', 'roles.id', '=', 'user_roles.role_id')
                        ->where('user_roles.school_id', $schoolId)
                        ->where('user_roles.is_active', true)
                        ->where('roles.code', '!=', 'student')
                        ->whereIn('user_roles.user_id', $staffUserIds)
                        ->select(['user_roles.user_id', 'roles.code as role_code'])
                        ->get()
                        ->groupBy(static fn ($row): int => (int) $row->user_id);
                }

                $schoolStaffPaginator->setCollection(
                    $schoolStaffPaginator->getCollection()->map(function (User $user) use ($roleRowsByUserId): array {
                        $rows = $roleRowsByUserId->get((int) $user->id, collect());
                ...
                $schoolStaffForRegistrar = $schoolStaffPaginator;
            } else {
            ...
            if ($request->query('partial') === 'registrar-verified' && $canRegistrarViewAllRecords) {
                return view('tenant.partials.enrollments-registrar-verified-section', [
                    'financeVerifiedForRegistrar' => $financeVerifiedForRegistrar,
                    'registrarVerifiedSearch' => $registrarVerifiedSearch,
                ]);
            }
            if ($request->query('partial') === 'registrar-clearances' && $canRegistrarViewAllRecords) {
                return view('tenant.partials.enrollments-registrar-clearances-section', [
                    'pendingClearancesForRegistrar' => $pendingClearancesForRegistrar,
                    'registrarClearanceSearch' => $registrarClearanceSearch,
                ]);
            }
            if ($request->query('partial') === 'registrar-class-groups' && $canRegistrarViewAllRecords) {
                return view('tenant.partials.enrollments-registrar-class-groups-section', [
                    'classGroupCapacityForRegistrar' => $classGroupCapacityForRegistrar,
                    'registrarClassGroupSearch' => $registrarClassGroupSearch,
                ]);
            }
            if ($request->query('partial') === 'registrar-inactive' && $canRegistrarViewAllRecords) {
                return view('tenant.partials.enrollments-registrar-inactive-section', [
                    'inactiveStudentAccountsForRegistrar' => $inactiveStudentAccountsForRegistrar,
                    'registrarInactiveSearch' => $registrarInactiveSearch,
                ]);
            }
            if ($request->query('partial') === 'registrar-all-students' && $canRegistrarViewAllRecords) {
                return view('tenant.partials.enrollments-registrar-all-students-section', [
                    'allStudentsForRegistrar' => $allStudentsForRegistrar,
                    'registrarAllStudentsSearch' => $registrarAllStudentsSearch,
                ]);
            }
            if ($request->query('partial') === 'registrar-staff' && $canRegistrarViewAllRecords) {
                return view('tenant.partials.enrollments-registrar-staff-section', [
            ...
            return view('tenant.enrollments', compact(
                ...
                'registrarVerifiedSearch',
                'registrarClearanceSearch',
                'registrarClassGroupSearch',
                'classGroupCapacityForRegistrar',
                'registrarInactiveSearch',
                'inactiveStudentAccountsForRegistrar',
                'registrarAllStudentsSearch',
                'allStudentsForRegistrar',
                'registrarStaffSearch',
        ...
        public function payBillingReview(Request $request, Billing $billing): View|\Illuminate\Http\RedirectResponse
        {
            ...
            $isTuitionBreakdown = ((string) $billing->charge_type === 'tuition');

            if ($isTuitionBreakdown) {
                $enrollments = Enrollment::query()
                ...
                $desc = trim((string) ($billing->description ?? ''));
                if ($desc === '') {
                    $desc = ucfirst(str_replace('_', ' ', (string) $billing->charge_type));
                }
                $typeLabel = strtoupper(str_replace('_', ' ', (string) $billing->charge_type));
            ...
            return view('tenant.payment-review', [
                'billing' => $billing,
                'breakdown' => $breakdown,
                'isTuitionBreakdown' => $isTuitionBreakdown,
                'totalDue' => $totalDue,
        ...
        public function cashierPage(Request $request): View
        {
            ...
            $pendingSearch = trim((string) $request->query('pending_q', ''));
            $pendingTypeFilter = $request->query('pending_type', 'all'); // all, tuition, enrollment, other
            $pendingSort = $request->query('pending_sort', 'latest'); // latest, oldest, amount_high, amount_low

            $confirmedSearch = trim((string) $request->query('confirmed_q', ''));
            $confirmedTypeFilter = $request->query('confirmed_type', 'all'); // all, tuition, enrollment, other
            $confirmedSort = $request->query('confirmed_sort', 'latest'); // latest, oldest, amount_high, amount_low
            ...
            $applyCashierFilters = function ($query, string $search, string $typeFilter, string $sort) {
                $query
                    ->when($typeFilter !== 'all', function ($query) use ($typeFilter): void {
                        $query->whereHas('billing', function ($billingQuery) use ($typeFilter): void {
                            if ($typeFilter === 'tuition') {
                                $billingQuery->where('charge_type', 'tuition');
                            } elseif ($typeFilter === 'enrollment') {
                                $billingQuery->where('charge_type', 'misc_fee');
                            } elseif ($typeFilter === 'other') {
                                $billingQuery->whereNotIn('charge_type', ['tuition', 'misc_fee']);
                            }
                        });
                    })
                    ->when($search !== '', function ($query) use ($search): void {
                        ...
                        $query->where(function ($inner) use ($like, $asInt): void {
                            $inner->whereHas('student', function ($studentQuery) use ($like): void {
                                $studentQuery->where('full_name', 'like', $like)
                            ...
                            if ($asInt !== null) {
                                $inner->orWhere('id', $asInt)
                                    ->orWhere('billing_id', $asInt);
                            }
                        });
                    });
            ...
            $pendingQuery = Payment::query()
                ...
                ->whereIn('status', ['submitted']);
            $pendingQuery = $applyCashierFilters($pendingQuery, $pendingSearch, (string) $pendingTypeFilter, (string) $pendingSort);

            $pendingPayments = $pendingQuery->paginate($perPage, ['*'], 'pending_page');

            $confirmedQuery = Payment::query()
                ->with(['billing', 'student'])
                ->whereHas('billing', function ($query) use ($schoolId): void {
                    $query->where('school_id', $schoolId);
                })
                ->where('status', 'verified');
            $confirmedQuery = $applyCashierFilters($confirmedQuery, $confirmedSearch, (string) $confirmedTypeFilter, (string) $confirmedSort);

            $confirmedPayments = $confirmedQuery->paginate($perPage, ['*'], 'confirmed_page');
            ...
            if ($partial === 'pending') {
                return view('tenant.partials.cashier-pending-section', [
            ...
            if ($partial === 'confirmed') {
                return view('tenant.partials.cashier-confirmed-section', [
                    'confirmedPayments' => $confirmedPayments,
                    'confirmedSearch' => $confirmedSearch,
                    'confirmedTypeFilter' => $confirmedTypeFilter,
                    'confirmedSort' => $confirmedSort,
                ]);
            }

            return view('tenant.cashier', [
                'pendingPayments' => $pendingPayments,
                'confirmedPayments' => $confirmedPayments,
                'autoApprovePaymentsEnabled' => $autoApprovePaymentsEnabled,
        ...
        public function adminPage(Request $request): View
        {
            ...
            $curriculumItems = ProgramCurriculumItem::query()
                ->with(['program.department', 'semester', 'subject.prerequisites:id,code,title'])
                ->where('school_id', $schoolId)
                ->orderBy('program_id')
                ->orderBy('year_level')
                ->orderBy('semester_id')
            ...
            $programProspectusesByProgramId = ProgramProspectus::query()
                ->where('school_id', $schoolId)
            ...
            return view('tenant.admin', compact(
                ...
                'programProspectusesByProgramId',
        ...
        public function updateSchoolPage(Request $request): RedirectResponse
        {
            ...
            $payload = $request->validate([
                ...
                'footer_logo_data' => ['nullable', 'string', 'max:10485760'],
                'footer_logo' => ['nullable', 'image', 'max:5120'],
                'remove_footer_logo' => ['nullable', 'in:1,yes,true'],
                ...
                'footer_address' => ['nullable', 'string', 'max:1500'],
                'footer_email' => ['nullable', 'email', 'max:255'],
                'footer_phone' => ['nullable', 'string', 'max:80'],
                'footer_copyright' => ['nullable', 'string', 'max:500'],
                'footer_link_labels' => ['nullable', 'array', 'max:12'],
                'footer_link_labels.*' => ['nullable', 'string', 'max:120'],
                'footer_link_urls' => ['nullable', 'array', 'max:12'],
                'footer_link_urls.*' => ['nullable', 'string', 'max:500'],
                'footer_social_facebook' => ['nullable', 'string', 'max:500'],
                'footer_social_instagram' => ['nullable', 'string', 'max:500'],
                'footer_social_x' => ['nullable', 'string', 'max:500'],
                'footer_social_youtube' => ['nullable', 'string', 'max:500'],
                'footer_social_website' => ['nullable', 'string', 'max:500'],
            ]);
            ...
            if (! empty($payload['remove_footer_logo'])) {
                if ($school->footer_logo_path && Storage::disk('public')->exists($school->footer_logo_path)) {
                    Storage::disk('public')->delete($school->footer_logo_path);
                }
                $school->footer_logo_path = null;
            } elseif (! empty($payload['footer_logo_data']) && preg_match('/^data:image\/(jpeg|png|gif|webp);base64,/', $payload['footer_logo_data'], $m)) {
                if ($school->footer_logo_path && Storage::disk('public')->exists($school->footer_logo_path)) {
                    Storage::disk('public')->delete($school->footer_logo_path);
                }
                $data = base64_decode(preg_replace('/^data:image\/\w+;base64,/', '', $payload['footer_logo_data']), true);
                if ($data !== false && strlen($data) > 0 && strlen($data) <= 5 * 1024 * 1024) {
                    $ext = $m[1] === 'jpeg' ? 'jpg' : $m[1];
                    $path = "{$dir}/footer-logo.{$ext}";
                    Storage::disk('public')->put($path, $data);
                    $school->footer_logo_path = $path;
                }
                ...
                $ext = $file ? ($file->getClientOriginalExtension() ?: 'jpg') : 'jpg';
                $path = $file ? $file->storeAs($dir, 'footer-logo.' . $ext, 'public') : null;
                if ($path) {
                    $school->footer_logo_path = $path;
                }
            }
            ...
            $profile->footer_title = $payload['footer_title'] ?? $profile->footer_title;
            $profile->footer_description = $payload['footer_description'] ?? $profile->footer_description;
            $profile->footer_address = $payload['footer_address'] ?? $profile->footer_address;
            $profile->footer_email = $payload['footer_email'] ?? $profile->footer_email;
            $profile->footer_phone = $payload['footer_phone'] ?? $profile->footer_phone;
            $profile->footer_copyright = $payload['footer_copyright'] ?? $profile->footer_copyright;
            $profile->footer_quick_links = $this->buildFooterQuickLinksFromRequest($request);
            $profile->footer_social_facebook = $this->normalizeOptionalHttpUrl($request->input('footer_social_facebook'), 'footer_social_facebook');
            $profile->footer_social_instagram = $this->normalizeOptionalHttpUrl($request->input('footer_social_instagram'), 'footer_social_instagram');
            $profile->footer_social_x = $this->normalizeOptionalHttpUrl($request->input('footer_social_x'), 'footer_social_x');
            $profile->footer_social_youtube = $this->normalizeOptionalHttpUrl($request->input('footer_social_youtube'), 'footer_social_youtube');
            $profile->footer_social_website = $this->normalizeOptionalHttpUrl($request->input('footer_social_website'), 'footer_social_website');
            $profile->save();
         ...
         */
        private function buildFooterQuickLinksFromRequest(Request $request): array
        {
            $labels = (array) $request->input('footer_link_labels', []);
            $urls = (array) $request->input('footer_link_urls', []);
            $out = [];
            $n = min(12, max(count($labels), count($urls)));
            for ($i = 0; $i < $n; $i++) {
                ...
                $u = $this->normalizeOptionalHttpUrl($urlRaw, "footer_link_urls.{$i}");
                if ($u === null) {
                    continue;
                }
                $out[] = ['label' => $label, 'url' => $u];
            }

            return $out;
        }

        private function normalizeOptionalHttpUrl(?string $input, string $fieldName): ?string
        {
            $t = trim((string) $input);
            if ($t === '') {
                return null;
            ...
            if (! filter_var($t, FILTER_VALIDATE_URL)) {
                throw ValidationException::withMessages([$fieldName => 'Enter a valid URL (e.g. https://example.com).']);
            }
        ...
        /**
         * Same visibility as School / Settings sidebar: school_admin role or school admin permissions.
         */
        private function authorizeSchoolHubAccess(Request $request): void
        {
            $permissionCodes = (array) session('permission_codes', []);
            $hasAnyPermission = static function (array $required) use ($permissionCodes): bool {
            ...
            $roleCodes = collect((array) ($request->attributes->get('role_codes') ?? []))
                ...
                ->filter(fn ($role): bool => $role !== '')
                ->values();
            $isSchoolAdmin = $roleCodes->contains('school_admin');
            $hasSchoolHubPermission = $hasAnyPermission([
                'school_admin.manage_staff',
                'school_admin.assign_roles',
                'school_admin.manage_curriculum',
            ]);
            abort_unless($isSchoolAdmin || $hasSchoolHubPermission, 403);
        }

        public function settingsAppearancePage(Request $request): View
        {
            $this->authorizeSchoolHubAccess($request);

            $schoolId = (int) $request->attributes->get('active_school_id');
            $school = School::query()->where('id', $schoolId)->firstOrFail();
            $appearance = $school->adminAppearanceConfig();
        ...
        public function updateSettingsAppearance(Request $request): RedirectResponse
        {
            $this->authorizeSchoolHubAccess($request);
            ...
            if ($request->boolean('reset_admin_appearance')) {
                $school->admin_appearance = null;
                $school->save();

                return redirect('/tenant/settings/appearance')->with('status', 'Dashboard appearance reset to default.');
            }

            $allowedPresets = ['default', 'green', 'blue', 'purple', 'dark'];
            $payload = $request->validate([
                'preset' => ['required', 'string', 'max:40', Rule::in($allowedPresets)],
                'primary_color' => ['required', 'regex:/^#[0-9A-Fa-f]{6}$/'],
                'sidebar_color' => ['required', 'regex:/^#[0-9A-Fa-f]{6}$/'],
                'accent_color' => ['required', 'regex:/^#[0-9A-Fa-f]{6}$/'],
            ]);

            $school->admin_appearance = [
                'preset' => $payload['preset'],
                'primary' => strtolower($payload['primary_color']),
                'sidebar' => strtolower($payload['sidebar_color']),
                'accent' => strtolower($payload['accent_color']),
            ];
            $school->save();

            return redirect('/tenant/settings/appearance')->with('status', 'Dashboard appearance updated.');
        }

        public function settingsAccountPage(Request $request): View
        {
            $this->authorizeSchoolHubAccess($request);
            ...
            $roleCodes = collect((array) ($request->attributes->get('role_codes') ?? session('role_codes', [])))
            ...
            return view('tenant.settings-account', [
                ...
                'accountUser' => $user,
                'accountRoleCodes' => $roleCodes,
            ]);
        }

        public function updateSettingsAccountEmail(Request $request): RedirectResponse
        {
            $this->authorizeSchoolHubAccess($request);
            ...
            if (! $user instanceof User) {
                return redirect('/tenant/settings/account')->with('error', 'Could not load your account.');
            }
            ...
            if (! Hash::check($payload['current_password'], (string) $user->password_hash)) {
                return redirect('/tenant/settings/account')->withErrors(['current_password' => 'Current password is incorrect.'])->withInput();
            }
            ...
            $exists = User::query()
                ...
                ->exists();
            if ($exists) {
                return redirect('/tenant/settings/account')->withErrors(['email' => 'That email is already in use.'])->withInput();
            }

            $user->email = $email;
            $user->save();

            return redirect('/tenant/settings/account')->with('status', 'Email updated.');
        }

        public function updateSettingsAccountPassword(Request $request): RedirectResponse
        {
            $this->authorizeSchoolHubAccess($request);
            ...
            if (! Hash::check($payload['current_password'], (string) $user->password_hash)) {
                return redirect('/tenant/settings/account')->withErrors(['current_password' => 'Current password is incorrect.']);
            }

            $user->password_hash = Hash::make($payload['new_password']);
            $user->save();

            return redirect('/tenant/settings/account')->with('status', 'Password updated.');
        }

        public function updateSettingsAccountAvatar(Request $request): RedirectResponse
        {
            // Student profile-avatar upload is self-service and should work
            // for any authenticated tenant user (including student role).
            if (! $request->session()->has('active_school_id')) {
                abort(403);
            }
            ...
            if (! $user instanceof User) {
                return redirect()->back()->with('error', 'Could not load your account.');
            }
            ...
            // Prefer direct public path so Hostinger/public_html setups work
            // even when storage symlink is missing.
            $extension = strtolower((string) $file->getClientOriginalExtension());
            if ($extension === '') {
                $extension = $file->extension() ?: 'jpg';
            }
            $filename = 'avatar-' . $user->id . '-' . time() . '-' . bin2hex(random_bytes(4)) . '.' . $extension;
            $relativePublicPath = 'images/profile-photos/' . $filename;
            $targetDir = public_path('images/profile-photos');
            ...
            try {
                ...
                $user->profile_photo_path = $relativePublicPath;
            } catch (\Throwable $e) {
                // Fallback for restrictive filesystem environments.
                $path = $file->store('users/profile-photos', 'public');
        ...
        public function viewSettingsAccountAvatar(Request $request)
        {
            if (! $request->session()->has('active_school_id')) {
                abort(403);
            }
            ...
            // Direct public path (works for Hostinger/public_html deployments).
            $publicCandidates = [];
            if (str_starts_with($normalized, 'images/')) {
                $publicCandidates[] = public_path($normalized);
                $publicCandidates[] = dirname(base_path()) . DIRECTORY_SEPARATOR . 'public_html' . DIRECTORY_SEPARATOR . str_replace('/', DIRECTORY_SEPARATOR, $normalized);
            }
            ...
            // Storage fallback (with and without accidental "storage/" prefix).
        ...
        public function adminAddCurriculumItem(Request $request): RedirectResponse
        {
            ...
            $currentYearLevel = (int) $payload['year_level'];
            $currentSemesterTermCode = strtoupper((string) ($semester->term_code ?? ''));
            $termOrderMap = [
                '1ST' => 1,
                '2ND' => 2,
                'SUMMER' => 3,
            ];
            $currentTermOrder = $termOrderMap[$currentSemesterTermCode] ?? 99;
            $earlierTermCodes = array_values(array_keys(array_filter(
                $termOrderMap,
                fn (int $order): bool => $order < $currentTermOrder
            )));

            // 1) Prerequisites validation:
            //    Every prerequisite subject must exist in the same program curriculum in a "previous"
            //    semester order (1ST before 2ND before SUMMER) OR any earlier year level.
            $prerequisiteIds = DB::table('subject_prerequisites')
                ->where('school_id', $schoolId)
                ->where('subject_id', (int) $subject->id)
            ...
            if (! empty($prerequisiteIds)) {
                ...
                foreach ($prerequisiteIds as $prerequisiteId) {
                    $hasPrerequisiteInEarlierTerm = ProgramCurriculumItem::query()
                        ->where('school_id', $schoolId)
                        ->where('program_id', (int) $payload['program_id'])
                        ->where('subject_id', (int) $prerequisiteId)
                        ->where(function ($q) use ($currentYearLevel, $earlierTermCodes, $schoolId): void {
                            $q->where('year_level', '<', $currentYearLevel);

                            if (! empty($earlierTermCodes)) {
                                $q->orWhere(function ($q2) use ($currentYearLevel, $earlierTermCodes, $schoolId): void {
                                    $q2->where('year_level', $currentYearLevel)
                                        ->whereIn('semester_id', function ($sub) use ($earlierTermCodes, $schoolId): void {
                                            $sub->select('id')
                                                ->from('semesters')
                                                ->where('school_id', $schoolId)
                                                ->whereIn('term_code', $earlierTermCodes);
                                        });
                                });
                            }
                        })
                    ...
                    if (! $hasPrerequisiteInEarlierTerm) {
                        $prereqCode = $prereqSubjects[$prerequisiteId]->code ?? ('Subject #' . $prerequisiteId);
                        throw ValidationException::withMessages([
                            'curriculum' => "Cannot add subject because prerequisite '{$prereqCode}' is not found in a previous semester/year level.",
                        ]);
                    }
                }
            }

            // 2) Max units per semester validation:
            //    Calculate total units for this program/year_level/semester and block if exceeding max_units_per_semester.
            $maxUnits = $program->max_units_per_semester !== null ? (float) $program->max_units_per_semester : null;
            if ($maxUnits !== null && $maxUnits > 0) {
                $existingTotalUnits = (float) DB::table('program_curriculum_items')
                    ->join('subjects', 'subjects.id', '=', 'program_curriculum_items.subject_id')
                    ->where('program_curriculum_items.school_id', $schoolId)
                    ->where('program_curriculum_items.program_id', (int) $payload['program_id'])
                    ->where('program_curriculum_items.semester_id', (int) $payload['semester_id'])
                    ->where('program_curriculum_items.year_level', $currentYearLevel)
                    ->sum('subjects.units');

                $alreadyMapped = ProgramCurriculumItem::query()->where([
                    'school_id' => $schoolId,
                ...
                $newTotalUnits = $alreadyMapped ? $existingTotalUnits : ($existingTotalUnits + (float) $subject->units);

                if ($newTotalUnits > $maxUnits) {
                    throw ValidationException::withMessages([
                        'curriculum' => "This curriculum exceeds the maximum allowed units per semester ({$newTotalUnits} > {$maxUnits}).",
                    ]);
                }
            }
        ...
        public function adminSaveProgramProspectus(Request $request): RedirectResponse
        {
            $schoolId = (int) $request->attributes->get('active_school_id');
            $actorUserId = (int) $request->attributes->get('actor_user_id');
            ...
            $payload = $request->validate([
                'program_id' => ['required', 'integer', 'exists:programs,id'],
                'prospectus_name' => ['required', 'string', 'max:180'],
                'status' => ['nullable', 'in:draft,active,archived'],
            ]);

            $programId = (int) $payload['program_id'];
            $program = Program::query()->where('school_id', $schoolId)->find($programId);
            if ($program === null) {
                return back()->withErrors(['prospectus' => 'Selected program does not belong to your school.'])->withInput();
            }

            $prospectus = ProgramProspectus::query()->create([
                'school_id' => $schoolId,
                'program_id' => $programId,
                'name' => (string) $payload['prospectus_name'],
                'status' => $payload['status'] ?? 'draft',
                'created_by_user_id' => $actorUserId,
            ]);

            $rows = ProgramCurriculumItem::query()
                ->where('school_id', $schoolId)
                ->where('program_id', $programId)
                ->where('status', 'active')
                ->get(['semester_id', 'subject_id', 'year_level'])
                ->map(function (ProgramCurriculumItem $item) use ($prospectus, $actorUserId): array {
                    return [
                        'program_prospectus_id' => (int) $prospectus->id,
            ...
            if (! empty($rows)) {
                ProgramProspectusItem::query()->insert($rows);
            }

            return back()->with('status', 'Prospectus saved for this curriculum.');
        }

        public function adminLoadProgramProspectusById(Request $request): RedirectResponse
        {
            $schoolId = (int) $request->attributes->get('active_school_id');
            $actorUserId = (int) $request->attributes->get('actor_user_id');

            $payload = $request->validate([
                'program_prospectus_id' => ['required', 'integer', 'exists:program_prospectuses,id'],
            ]);

            $prospectusId = (int) $payload['program_prospectus_id'];
            $prospectus = ProgramProspectus::query()->find($prospectusId);
            if ($prospectus === null || (int) $prospectus->school_id !== $schoolId) {
                return back()->withErrors(['prospectus' => 'Prospectus does not belong to your school.'])->withInput();
            }

            $programId = (int) $prospectus->program_id;

            // Replace the working curriculum with the snapshot items.
            ProgramCurriculumItem::query()
                ->where('school_id', $schoolId)
                ->where('program_id', $programId)
                ->delete();

            $items = ProgramProspectusItem::query()
                ->where('program_prospectus_id', $prospectusId)
                ->where('status', 'active')
                ->get(['semester_id', 'subject_id', 'year_level']);

            if ($items->isNotEmpty()) {
                $rows = $items->map(function (ProgramProspectusItem $item) use ($schoolId, $programId, $actorUserId): array {
                    return [
                        'school_id' => $schoolId,
                        'program_id' => $programId,
                        'semester_id' => (int) $item->semester_id,
                ...
                ProgramCurriculumItem::query()->insert($rows);
            }

            return back()->with('status', 'Prospectus loaded into the current working curriculum.');
        }

        public function adminResetProgramCurriculum(Request $request, Program $program): RedirectResponse
        {
            $schoolId = (int) $request->attributes->get('active_school_id');

            if ((int) $program->school_id !== $schoolId) {
                return back()->withErrors(['curriculum' => 'Program does not belong to your school.'])->withInput();
            }
            ...
            return back()->with('status', 'Working curriculum cleared. Add subjects again, then save changes.');
        }
